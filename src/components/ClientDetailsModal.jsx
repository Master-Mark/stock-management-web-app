...
const ClientDetailsModal = ({ isOpen, onClose, client, onEdit }) => {
  ...
  const displayName = client.type === 'Individual' 
          ...
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                {client.type === 'Individual' ? <User className="w-6 h-6" /> : <Building2 className="w-6 h-6" />}
              </div>
              <div>
                <DialogTitle className="text-xl mb-1">{displayName}</DialogTitle>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <div className="bg-card rounded-lg border border-border px-4">
                {client.type === 'Business' && <DetailRow label="Contact Person" value={client.contactPerson} />}
                <DetailRow label="Email" value={client.email} />
                <DetailRow label="Phone" value={client.phone} />
                <DetailRow label="Alternate Phone" value={client.alternatePhone} />
              </div>
            </div>
            ...
            <div>
              <h4 className="font-semibold mb-3 text-foreground">Order History</h4>
              <div className="bg-card rounded-lg border border-border px-4">
                <DetailRow label="Total Orders" value={client.totalOrders} />
                <DetailRow label="Total Spent" value={`$${Number(client.totalSpent || 0).toFixed(2)}`} />
              </div>
            </div>
