
import React from 'react';
import {
  Dialog,
  DialogContent,
  DialogHeader,
  DialogTitle,
} from "@/components/ui/dialog";
import { Button } from '@/components/ui/button';
import { Badge } from '@/components/ui/badge';
import { Edit, Truck } from 'lucide-react';
import { ScrollArea } from '@/components/ui/scroll-area';

const DetailRow = ({ label, value }) => (
  <div className="flex flex-col sm:flex-row sm:justify-between py-2 border-b border-border last:border-0">
    <span className="text-sm text-muted-foreground font-medium">{label}</span>
    <span className="text-sm text-foreground sm:text-right sm:max-w-[60%]">{value || '-'}</span>
  </div>
);

const SupplierDetailsModal = ({ isOpen, onClose, supplier, onEdit }) => {
  if (!supplier) return null;

  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="px-6 py-4 border-b border-border">
          <div className="flex justify-between items-start">
            <div className="flex items-center gap-3">
              <div className="w-12 h-12 rounded-full bg-primary/10 flex items-center justify-center text-primary">
                <Truck className="w-6 h-6" />
              </div>
              <div>
                <DialogTitle className="text-xl mb-1">{supplier.companyName}</DialogTitle>
                <div className="flex items-center gap-2">
                  <p className="font-mono text-sm text-muted-foreground">{supplier.id}</p>
                  {supplier.isActive ? (
                    <Badge className="bg-green-100 text-green-800 hover:bg-green-100">Active</Badge>
                  ) : (
                    <Badge variant="secondary">Inactive</Badge>
                  )}
                </div>
              </div>
            </div>
            <Button variant="outline" size="sm" onClick={() => onEdit(supplier)} className="gap-2">
              <Edit className="w-4 h-4" /> Edit
            </Button>
          </div>
        </DialogHeader>
        
        <ScrollArea className="flex-1 px-6 py-4">
          <div className="space-y-6">
            
            <div>
              <h4 className="font-semibold mb-3 text-foreground">Contact Information</h4>
              <div className="bg-card rounded-lg border border-border px-4">
                <DetailRow label="Contact Person" value={supplier.contactPerson} />
                <DetailRow label="Position" value={supplier.contactPersonPosition} />
                <DetailRow label="Email" value={supplier.email} />
                <DetailRow label="Phone" value={supplier.phone} />
                <DetailRow label="Alternate Phone" value={supplier.alternatePhone} />
                <DetailRow label="Website" value={supplier.website} />
              </div>
            </div>

            <div>
              <h4 className="font-semibold mb-3 text-foreground">Address</h4>
              <div className="bg-card rounded-lg border border-border px-4">
                <DetailRow label="Address Line 1" value={supplier.addressLine1} />
                <DetailRow label="Address Line 2" value={supplier.addressLine2} />
                <DetailRow label="City" value={supplier.city} />
                <DetailRow label="State / Province" value={supplier.province} />
                <DetailRow label="Postal Code" value={supplier.postalCode} />
                <DetailRow label="Country" value={supplier.country} />
              </div>
            </div>

            <div>
              <h4 className="font-semibold mb-3 text-foreground">Business Terms</h4>
              <div className="bg-card rounded-lg border border-border px-4">
                <DetailRow label="Supplier Code" value={supplier.supplierCode} />
                <DetailRow label="Payment Terms" value={supplier.paymentTerms} />
                <DetailRow label="Lead Time" value={`${supplier.leadTimeDays} days`} />
                <DetailRow label="Preferred Contact" value={supplier.preferredContactMethod} />
              </div>
            </div>

            {supplier.notes && (
              <div>
                <h4 className="font-semibold mb-3 text-foreground">Notes</h4>
                <div className="bg-muted/50 rounded-lg p-4 text-sm text-foreground">
                  {supplier.notes}
                </div>
              </div>
            )}

          </div>
        </ScrollArea>
      </DialogContent>
    </Dialog>
  );
};

export default SupplierDetailsModal;
