            ...
            products.map((product) => (
              <TableRow key={product.id} className="hover:bg-muted/50 transition-colors">
                ...
                <TableCell className="text-right">
                  <div className="flex flex-col">
                    <span className="text-xs text-muted-foreground line-through">${Number(product.supplierCost).toFixed(2)}</span>
                ...
                <TableCell className="text-center">
                  <div className="flex items-center justify-center gap-2">
                    ...
                    {product.quantity <= product.minStockLevel && !product.isSpecialOrder && (
                      <AlertTriangle className="w-4 h-4 text-destructive" title="Low Stock" />
                    )}
                  </div>
                </TableCell>
